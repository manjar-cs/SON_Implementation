...
if __name__== '__main__':
    ...
    # Forming the baskets
    rdd_2 = rdd.filter(lambda x:x!=header).map(lambda x:x.split(",")).map(lambda x:(x[0],x[1])).groupByKey().mapValues(set).filter(lambda x: len(x[1])>threshold).map(lambda x:x[1]).cache()
    ...
    fname=sys.argv[4]+"manjari_chinniyansubramani_task2.txt"
...
if __name__== '__main__':
    ...
    rdd = sc.textFile("/Users/manjari/PycharmProjects/553_NEW/yelp.csv")
    header = rdd.first()

    support=50


    rdd_2 = rdd.filter(lambda x:x!=header).map(lambda x:x.split(",")).map(lambda x:(x[0],x[1])).groupByKey().mapValues(set).filter(lambda x: len(x[1])>70).map(lambda x:x[1]).cache()
    rdd_collect=rdd_2.collect()
    num_baskets=len(rdd_collect)
    rdd_test = rdd_2.mapPartitions(apriori).flatMap(lambda x:x).reduceByKey(lambda x,y:x+y).map(phase2).flatMap(lambda x: x).filter(lambda x: x[1] >= support).map(lambda x: x[0]).collect()
    print("final", sorted(rdd_test, key=len))
